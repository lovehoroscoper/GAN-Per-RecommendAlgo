...
# change resize_height = 128, crop = False


def get_image(image_path, input_height, input_width,
              resize_height=128, resize_width=64, crop=False, grayscale=False):
...
# change resize_height = 128, may not use this function(center_crop).


def center_crop(x, crop_h, crop_w, resize_h=128, resize_w=64):
    if crop_w is None:
        crop_w = crop_h
    h, w = x.shape[:2]
...
def transform(image, input_height, input_width,
              resize_height=128, resize_width=64, crop=False):
    if crop:
